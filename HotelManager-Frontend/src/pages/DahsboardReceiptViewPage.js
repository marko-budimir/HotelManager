...
const DashboardReceiptViewPage = () => {
    ...
    const calculateNumberOfNights = (checkInDate, checkOutDate) => {
        ...
        const numberOfNights = Math.round((endDate - startDate) / oneDayMilliseconds);
        return numberOfNights;
    };

    const subtotalServicePrice = services.reduce((total, service) => total + (service.price * service.quantity), 0);
    const totalDiscountedPrice = receipt ? receipt.totalPrice * (1 - (receipt.percent / 100)) : 0;
    const totalPrice = (totalDiscountedPrice || receipt?.totalPrice || 0) + subtotalServicePrice;

    const handleSendReceipt = async () => {
        try{
            const isSent = await api_dashboard_invoice.sendDashboardInvoice(receiptId);
            if(isSent){
        ...
        <div className="dashboard-receipt-view-page page">
            ...
            <div className="container">
                ...
                <div className="dashboard-receipt-view-info">
                    <h3 className="dashboard-receipt-view-info-title">Receipt Overview</h3>
                    {receipt && (
                        <>
                            ...
                            <p>Number of Nights: {calculateNumberOfNights(receipt.checkInDate, receipt.checkOutDate)}</p>
                            <p>Price per Night: {formatCurrency(receipt.pricePerNight)}</p>
                            <p>Subtotal: {formatCurrency(receipt.totalPrice)}</p>
                            {receipt.discountId && (
                                <>
                                    <p>Discount: {receipt.code} ({receipt.percent}%)</p>
                                    <p>Subtotal with discount: {formatCurrency(receipt.totalPrice * (1 - receipt.percent / 100))}</p>
                                </>
                            )}
                            {services.length > 0 &&
                                <>
                                    ...
                                    <p>
                                        Subtotal: {formatCurrency(subtotalServicePrice)}
                                    </p>
                                </>
                            }
                ...
                <div className="dashboard-receipt-view-buttons">
                    <button className="dashboard-receipt-view-button-paid" onClick={handleSendReceipt}>Paid</button>
                    <button className="dashboard-receipt-view-button-edit" onClick={handleEditReceipt}>Edit</button>
                </div>
            </div>
        </div>
    );
