...
export default function ReservationForm() {
  ...
  useEffect(() => {
    if (id) {
      setMode('view');      
      apiReservation.getByIdReservation(id)
      .then(async response => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
        ...
        if (invoices && invoices.length > 0) {
            ...
            const isUpdated = await apiReservation.updateReservation(id, invoiceId,reservationData);
            if (isUpdated) {
                handleSuccess();
                setMode('view');
            } else {
  ...
  return (
    <div>
      ...
      <form id="reservationForm" onSubmit={handleFormSubmit}>
        ...
        <input type="date" id="checkInDate" name="checkInDate" value={reservationData.checkInDate} disabled={mode === 'view'} onChange={(e) => setReservationData({ ...reservationData, checkInDate: e.target.value })} /><br /><br />

        <label htmlFor="checkOutDate">Check-Out Date: </label><br />
        <input type="date" id="checkOutDate" name="checkOutDate" value={reservationData.checkOutDate} disabled={mode === 'view'} onChange={(e) => setReservationData({ ...reservationData, checkOutDate: e.target.value })} /><br /><br />

        <label htmlFor="pricePerNight">Price per Night: </label><br />
        <input type="number" id="pricePerNight" name="pricePerNight" value={reservationData.pricePerNight} disabled={mode === 'view'} onChange={(e) => setReservationData({ ...reservationData, pricePerNight: e.target.value })} /><br /><br />
